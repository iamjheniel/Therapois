...
export class DeceasedPage {
  ...
  async openPatientForm(id: number): Promise<void> {
    await this.page.setViewportSize({ width: 1920, height: 1080 });
    await this.page.goto(`${this.baseUrl}/patient-management/${id}/edit?id=${id}`, { waitUntil: 'domcontentloaded' });
  ...
  async openPatientManagement(): Promise<void> {
    await this.page.setViewportSize({ width: 1920, height: 1080 });
    await this.page.goto(`${this.baseUrl}/patient-management`, { waitUntil: 'domcontentloaded' });
  ...
  async showDeceased(): Promise<void> {
    ...
    await this.page.getByText('Verstorben: Alle anzeigen', { exact: true }).filter({ visible: true }).first()
  ...
  async openCreateVo(): Promise<void> {
    await this.page.setViewportSize({ width: 1920, height: 1080 });
    await this.page.goto(`${this.baseUrl}/vo-management/add`, { waitUntil: 'domcontentloaded' });
    await this.page.waitForTimeout(4000);
  }
  ...
  async selectVoPatient(name: string): Promise<boolean> {
    ...
    await opt.click({ force: true, timeout: 6000 }).catch(() => {});
    await this.page.waitForTimeout(2500);
    return true;
  }
