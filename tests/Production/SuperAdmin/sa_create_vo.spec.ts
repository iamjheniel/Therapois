...
// End-to-end "Create VO" (VO erstellen) happy path for a SuperAdmin — written to be
...
//    Vorgänger-VO gate, or the /practices API being unavailable), it does NOT fail — it
//    falls back to asserting the data-independent form contract (Praxis required, Doctor
//    optional) and marks the test skipped with the reason.
//
// Note: on a healthy environment this creates a real VO. That is data-creating by design.

test.describe('SuperAdmin — Create VO (end-to-end)', () => {
  test(
    'Creates a VO through the full validation flow (or verifies the form contract)',
    { tag: ['@SuperAdmin', '@CreateVO'] },
    async ({ page }) => {
