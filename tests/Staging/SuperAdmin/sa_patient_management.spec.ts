...
test.describe('Super Admin - Patient Management', () => {
  ...
  test(
    ...
    async ({ page }) => {
      ...
      await pm.openPatientManagement();
      await pm.search('Martina');
      await pm.expectPatientVisible('Martina');
    }
  );
  ...
  test(
    ...
    async ({ page }) => {
      ...
      await pm.openPatientManagement();
      // Use a known patient number from staging QA data
  ...
  test(
    ...
    async ({ page }) => {
      ...
      await pm.openPatientManagement();
      await pm.search('Martina');
      await pm.openPatientDetail('Martina');

      // Verify a detail panel/section is shown
      await expect(page.locator('#root')).toContainText(
        /Patientendetails|Patient Details|Martina/i,
        { timeout: 15_000 }
      );
    }
  );
