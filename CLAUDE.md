...
Pages/                              # Page Object Model classes
  admin/
    ...
    admin.letter-layout.page.ts     # ePIN letter GEOMETRY: Vorabinformation generation + page count, footer column widths, copayment FAQ font size, /preview order forms, Infoblatt (RC 3.11.3 #3522/#3523/#3524/#3525)
  base/
  ...
  superadmin/
    ...
    sa.therapist-performance.page.ts # Efficiency traffic light: /therapist-performance rows (qualifyingDays, window, status) + /kpis/management/efficiency-buckets (RC 3.11.2 #3486)
  ...
  util/
    ...
    pdf-layout.ts                   # POSITIONED text: page tree, CTM/text matrix, font sizes, filled rects — page counts and column geometry `pdf-text` deliberately drops (RC 3.11.3)
...
| `admin_vorabinfo_one_page.spec.ts` | `@Admin @VorabinfoOnePage @ReadOnly @Mutating` | RC 3.11.3 (#3522) a Vorabinformation with ≤3 prescribed treatments must fit ONE page, closing included. **The fix IS deployed on staging** (verified 2026-08-27) so all five tests run for real. No version string could establish that — the frontend bundle still reports `3.11.0` while the API already renders 3.11.2's banner — so deployment is decided by READING THE PDF. Patient 7793's notice archive is the timeline: its Blanko variant was **2 pages with the closing stranded on page 2 on all 11 renders from 2026-08-13 to 2026-08-26, and 1 page from 2026-08-27**; the body genuinely tightened rather than gaining room, because the same deploy ADDED the banner (which pushes toward more pages) and the letter still got shorter. AC1/AC2 assert 1 page + closing on p1 for three fixtures in both variants (each prints exactly **3 treatment rows**, against patient 7793's **6 rows → 2 pages**, the ticket's out-of-scope case); AC3 pins the DIN 5008 anchors, which are **identical on every render ever made** (sender y=729.42, date y=521.67, title y=493.92) while only the salutation moved (446.67 → 454.92) — that shift *is* the compression. **Trap: patient 7793 is NOT an AC1 fixture** (6 treatments → legitimately 2 pages); AC1 needs `THREE_TREATMENT_PATIENTS` (8474/8472/8468), picked from the 253 active VOs with three by requiring exactly one PT VO so the table is unambiguous. **Mutating** — only a fresh render shows the current template. Uses `Pages/admin/admin.letter-layout.page.ts` |
| `admin_vorabinfo_footer.spec.ts` | `@Admin @LetterFooter @ReadOnly @Mutating` | RC 3.11.3 (#3525) the Vorabinformation hides the bank-details column (#3227) and the remaining four must fill the width evenly. **Deployed; AC1/AC3 pass, AC2 is `fixme`'d on a regression the fix introduced.** All geometry is read on a *derived* scale: the invoice footer's five columns measure 0/90.6/172.7/246.3/370.9pt, which over **453pt** is 0/20/38/54/82% — exactly the 20/18/16/28/18 the shared partial declares, so 453 is the measured content width. **The ticket's premise needs correcting:** it describes the bug as "4 columns totalling 72%, left-aligned", but the browser renormalised the declared 20/18/16/18 across the freed width (20/72=27.8%…), and every archived render through 2026-08-26 measures offsets **0/27.6/52.6/75%** — filling the width but UNEVENLY. Today: **0/25/50/75%**. A spec written to the "72%" wording would have asserted something that was never true. **Finding (AC2):** the even split NARROWED column 1 from 27.8% (126pt) to 25% (113pt), so "Curano Berlin-Brandenburg 2 GmbH" — one line on all 23 renders through 2026-08-26 — now breaks across two ("Curano Berlin-Brandenburg 2" / "GmbH") on all 3 renders from 2026-08-27. The AC's own bar ("matching the invoices") doesn't settle it either: the PKV invoice footer wraps its first column across FOUR lines. → `fixme` for a product decision, with an accompanying `@ReadOnly` evidence test that prints every render's column width beside its wrap count. AC4 (Therapy Report) → `fixme`, no reachable PDF for this role. Uses `Pages/admin/admin.letter-layout.page.ts` |
| `admin_order_form_banner.spec.ts` | `@Admin @OrderFormBanner @ReadOnly` | RC 3.11.3 (#3524) the rebrand banner belongs on both order forms and never on the Infoblatt; the initial order form's greeting loses its second comma. **AC1/AC2/AC5/AC6 pass on staging.** Driven through **`POST /preview`** — what the CRM modal itself calls (`downloadAsBlob('preview', {template, data})`) — rather than the CRM UI, for two reasons: no practice in the first 25 CRM rows has initial orders on staging, and, more importantly, the endpoint lets BOTH `address` branches render. That is load-bearing for AC5: the double comma came from `{% if address != 'ER' %},{% endif %}`, so the **practice-address** branch is the one that used to break and an 'ER'-only test would have passed against the bug. Needs no `crm-serial` lock (it never touches the shared practice). **AC4 is `fixme`'d for a specific reason, not "no data":** the banner only reached staging on 2026-08-27 and the NEWEST Infoblatt was rendered 2026-07-30, so every one predates the banner and finding none proves nothing. Measured: 73 ib_records (28 signed/12 for_relative_signing/9 archived/1 deleted); of the 50 newest, 15 are downloadable (all 2p, 2026-07-21…30, banner=false) and 34 answer **404 from S3**. An accompanying evidence test prints that inventory. AC3 → `fixme`: all 7 entities are `isRebranded: true` and no banner switch is exposed; flagged that `/preview` shows the banner for a payload naming no entity at all, so AC3 may fail once a non-Curano entity exists. Uses `Pages/admin/admin.letter-layout.page.ts` |
...
| `sa_efficiency_five_day_window.spec.ts` | `@SuperAdmin @EfficiencyWindow @FiveDayWindow @ReadOnly` | RC 3.11.2 (#3486) the efficiency traffic light must be a 5-day snapshot again, after #3183 dropped the 5-day cap and let the window grow to every qualifying day in the 84-day scan (Kian's report: 32/41/51 days). **The fix IS in effect on staging** — `qualifyingDays` peaks at exactly 5 across all 163 therapists ({0: 37, 3: 1, 5: 125}), so these run for real as the regression guard. **Read-only — every request a GET**; the traffic light's data is an API resource (`/therapist-performance` carries `qualifyingDays`, `windowStart`/`windowEnd`, `efficiencyPercent`, `performanceStatus`), so the tooltip is asserted from the payload instead of hovering 163 tooltips. AC1 also requires therapists to sit AT the cap (125 do), or a broken cap would pass on sparse data alone. AC5 compares all three surfaces: `/kpis/management/efficiency-buckets` (note the `/management` segment — bare `/kpis/efficiency-buckets` 404s) maps **1:1 with zero mismatches** across the 161 therapists on both — rot↔needs_attention 119, gruen↔on_track 2, gelb↔monitor 5, grau↔unassigned 35 — plus the Auslastung header counts (119/5/2/37). **Traps:** the two collections are different populations (236 bucket rows incl. inactive vs 163 perf rows), so compare over the intersection by therapist id; `efficiencyPercent` is **omitted, not zeroed**, at 0 qualifying days; and the Therapeuten-Gesundheit filter needs no API param — it is client-side over the `itemsPerPage=10000` read. **Finding:** AC4's day count holds everywhere, but its *date range* half does not — 14 of 126 windows span >14 calendar days, up to 59 (Alexandra Schöner: 5 days over 2026-06-08…08-05), a direct consequence of the "skip no-data days" rule #3486 keeps from #3183 → `fixme`'d for a product decision. Uses `Pages/superadmin/sa.therapist-performance.page.ts` |
...
| `sa_false_payment_matching.spec.ts` | `@SuperAdmin @FalsePaymentMatch @ReadOnly` | RC 3.11.2 (#3499) an old-format invoice must only be auto-marked paid when a cleared bank item matches BOTH its invoice number AND its debtor account (`collectClearedMatches()` keyed on the number alone; 36 of 43 old-format PKV invoices were falsely flipped to paid in production on 2026-08-25). **Read-only — every request a GET; nothing triggers a payment check.** **The matching rule itself is unverifiable from a client in ANY environment**, and not only because staging's DATEV is off: there is no debtor-account surface at all (`/datev_debtors`, `/debtors`, `/accounts_receivable`, `/bank_items` all **404**; the invoice payload carries no debtor field, the patient only `insuranceNumber`) and no cleared-bank-item surface, so both sides of the match are invisible → AC1/AC2/AC3-outcome and AC5/AC6 `fixme`'d with that evidence, as the ticket's own Testing Guidance directs. What IS asserted: the exposure baseline (44 old-format PKV, 43 `overdue` + 1 `to_send_to_dc`, **0 paid**, `datevSyncStatus` null on all — so a paid one here would mean DATEV got switched on); the ticket's fixtures confirmed live (426-16 = Dr. Ursula Grünwald-Schuller EUR 917.90, 426-14 = EUR 378.00); AC4's disjointness (no R-format number satisfies the old-format rule, which is what keeps AC4 true by construction); and a **post-deploy watch** designed to re-run against production around the first nightly window. **The collision surface is the headline finding: 34 of 44 exposed invoices share their amount with another booking in the 537-invoice book — EUR 630 occurs 38 times (14 of the 44 sit on it), EUR 378 16 times, EUR 917.90 9 times**, which is why amount/reference-only matching cannot be safe and is consistent with the 36-of-43 false-flip rate. Uses `Pages/superadmin/sa.datev-old-format.page.ts` (extended with `amountCollisionsWithin` / `amountFrequency`) |
| `sa_copayment_faq_font.spec.ts` | `@SuperAdmin @CopaymentFaq @ReadOnly @Mutating` | RC 3.11.3 (#3523) the GKV copayment invoice's FAQ page must use the largest text size that still keeps all 10 Q&A on ONE page. **Deployed; passes.** The numbers match the ticket exactly: the FAQ body measures **6.75pt** on every stored render — which IS the ticket's "9px" (9px = 6.75pt at 96dpi) — and **9pt (12px)** on a copy regenerated today, +33%, with all 10 questions still on one page and content filling 582pt of the page instead of 434pt. **Runs as Super Admin**, because the regeneration endpoint does. A regeneration is unavoidable: `GET /invoices/{id}/download` is served from the STORED PDF (#3332), so every existing invoice shows the size it was rendered with. The one mutating test uses the documented "Rechnung neu erstellen" → "Entwurf ersetzen" path against a `not_sent` DRAFT only — replaced in place, number kept, no Storno, the same write `sa_invoice_stored_pdfs.spec.ts` AC2 makes. **Traps:** an old invoice can have a THIRD page carrying nothing but the footer (two 2026-08-13 renders do), so every assertion locates the FAQ page **by content** (the page holding ≥5 numbered questions), never by index; and AC2's "one step larger would overflow" is not producible from a client — the size is not parameterisable — so the remaining headroom is reported rather than asserted. AC3 regression-checks that page 1 still renders its 8.25pt body across renders spanning 2026-08-13…27. Uses `Pages/admin/admin.letter-layout.page.ts` + `Pages/superadmin/sa.invoice-pdfs.page.ts` |
...
## The app version does NOT track the API templates (2026-08-27)

The frontend bundle still reports **`3.11.0`** (readable from a `version` key in the JS, since the
sidebar renders no version string), while the API already serves **RC 3.11.2's rebrand banner and
all four RC 3.11.3 letter changes**. Every one of #3522/#3523/#3524/#3525 is a backend Twig
template, deployed independently of the bundle — so **never gate a document-rendering test on an
app version**. Read the rendered PDF instead; the four RC 3.11.3 specs each open with a deployment
probe that does exactly that.

An API deploy landed on staging between **2026-08-26 01:30 and 2026-08-27 06:11 UTC**, and it is
precisely datable because a patient's Vorabinformation archive is a per-render timeline of the
template: patient 7793's renders are banner-free with the salutation at y=446.67 up to 2026-08-26,
and carry the banner with the salutation at y=454.92 from 2026-08-27. Two consequences worth
knowing:

- **`admin_rebranding_banner.spec.ts`'s `fixme`'d ACs can be revisited.** That spec was written when
  #3481 was NOT deployed; the banner now renders on a freshly generated Vorabinformation, PKV
  invoice, GKV copayment invoice and Storno.
- **Stored PDFs are never re-rendered.** Invoice downloads come from the store (#3332) and a notice
  is archived at generation, so re-reading an existing document shows the OLD layout. Anything
  asserting a rendering change must GENERATE (a notice) or REGENERATE (a `not_sent` invoice draft).
...
## Waiting: never sleep a guess (`Pages/util/settle.ts`)

`waitForTimeout` is simultaneously too slow (it is paid in full even when the app was ready
immediately) and too short (it expires mid-request under load, and the caller then reads the
PREVIOUS state — which looks exactly like a product bug). Four helpers cover almost every case:

| Helper | Use it for |
|---|---|
| `settleAfter(page, action, opts)` | An action that fires a request. Waits for the requests it triggered to come back, then for the DOM to stop moving. **Must WRAP the action** — its listeners attach inside the call, so running it afterwards misses the very requests it should wait for and silently falls through to the client-side probe path. |
| `waitForStable(locator, opts)` | A streaming list that has content but may still be appending. The caller must first establish there IS content (a `waitFor({ state: 'visible' })`); this only answers "has it stopped growing". |
| `waitForOpen(locator, ms)` | A portalled dropdown/dialog appearing. Returns a boolean, never throws. |
| `waitForAuthState(page, ms)` | An API-driven spec, right after `goto`. `auth-state` (the bearer token every `Pages/` API helper reads) is restored from `storageState` as the origin loads, so it is readable on the first poll — the `goto` + `waitForTimeout(5000)` pairs this replaced were waiting for nothing. |

Three sleeps are still legitimate and are deliberately kept:

1. **Retry-loop intervals** — the sleep only runs when the previous attempt failed, so the happy
   path never pays it (`CRMBasePage.ensurePracticeRowsVisible`, `AppPage.navTo`).
2. **Observation windows**, where the DURATION IS THE ASSERTION. `offline_cache_quota.spec.ts`
   waits 45–60 s to claim "at most one Sentry event per session" against a build that emitted one
   per ~3 s tick; the Flow Boards access-gate specs wait 8 s to claim a board rendered *nothing*.
   Shortening these buys time by weakening the test. Don't.
3. **Backend-propagation waits**, where the next read is an API call whose result changes
   asynchronously (`sa_retroactive_price_recompute.spec.ts`).

**Bound every action that can meet a disabled control.** `actionTimeout` is 0 project-wide, so a
bare `.click()` on a `disabled` button does not fail — it retries actionability forever and spends
the whole 90 s test budget before reporting a generic timeout naming only a line number. See
`clickSave()` in `tests/Staging/Therapist/document_treatment.spec.ts`: asserting `toBeEnabled` first
turns a 90 s hang into a ~10 s failure that says which row was in an undocumentable state.

## Data gates: prove an environment fact once (`tests/fixtures/data-gate.ts`)

Several files are gated on a condition belonging to the ENVIRONMENT, not the test — "no patient on
this board exposes an IB signer dialog". The gate is evaluated inside each test, but *reaching* it
costs a navigation plus a board search in `beforeEach`, so a closed gate makes every test in the
file pay full setup and then skip.

Measured on the Therapist project: **42% of wall clock (480 s of 1,144 s) was spent inside tests
that ended up skipped**, and the four IB spec files burned ~105 s re-proving one unchanged fact
across 14 tests.

`DataGate` caches the closed verdict for the rest of the file: `gate.skipIfKnownClosed()` at the top
of `beforeEach` (before the navigation), `gate.apply(await probe(), 'detail')` in place of
`test.skip(!probe, …)`. Only the NEGATIVE verdict is cached, and only within a file — an open gate
is re-probed every time, because a gate that was open can legitimately close mid-file. It is sound
only in a `serial` describe. **Do not use it to paper over a flaky selector**: if the probe is
closed because a locator is wrong, this hides the breakage behind a fast skip instead of a slow one.

## Measuring a change to the suite

**Staging response time varies by 4x between runs, so end-to-end wall clock cannot demonstrate a
speedup.** Measured directly: `therapist_board_toolbar.spec.ts`, with its code unchanged, took
**40.4 s, then 161.1 s, then 93.1 s** across three runs. Any before/after comparison smaller than
that is noise.

Judge a change on what is deterministic instead — the number of page reloads, navigations, skipped
tests reaching setup, or seconds of flat sleep on the happy path — and use per-test durations from
`--reporter=json` only for effects large enough to survive the variance (the IB files' 8 s → 0.3 s
per skipped test did; a 15% file-level change would not).
